...
public class LayerFront : MonoBehaviour
{
    ...
    public void SetTiles(List<Board.DoorPos> doors, TileAsset tiles, int floor, bool isBoss, int bossDoor)
    {
        if (isBoss)
        {
            if (doors.Contains(Board.DoorPos.Up))
                tilemap.SetTile(new Vector3Int(0, 6, 0), tiles.Tiles[1 + (floor - 1) * 18 + 11]);

            if (doors.Contains(Board.DoorPos.Right))
                tilemap.SetTile(new Vector3Int(8, 1, 0), tiles.Tiles[2 + (floor - 1) * 18 + 11]);

            if (doors.Contains(Board.DoorPos.Left))
                tilemap.SetTile(new Vector3Int(-8, 1, 0), tiles.Tiles[7 + (floor - 1) * 18 + 9]);
        }
        else
        {
            if (bossDoor == -1)
            {
                if (doors.Contains(Board.DoorPos.Up))
                    tilemap.SetTile(new Vector3Int(0, 6, 0), tiles.Tiles[1 + (floor - 1) * 18]);

                if (doors.Contains(Board.DoorPos.Right))
                    tilemap.SetTile(new Vector3Int(8, 1, 0), tiles.Tiles[2 + (floor - 1) * 18]);

                if (doors.Contains(Board.DoorPos.Left))
                    tilemap.SetTile(new Vector3Int(-8, 1, 0), tiles.Tiles[7 + (floor - 1) * 18]);
            }
            else
            {
                if (bossDoor == 1)
    ...
    public void ClearTiles(List<Board.DoorPos> doors, TileAsset tiles, int floor)
    {
        ...
        if (doors.Contains(Board.DoorPos.Right))
            tilemap.SetTile(new Vector3Int(8, 1, 0), tiles.Tiles[4 + (floor - 1) * 18]);

        if (doors.Contains(Board.DoorPos.Left))
            tilemap.SetTile(new Vector3Int(-8, 1, 0), tiles.Tiles[9 + (floor - 1) * 18]);
    }
}
